...
All connections are read-only, even if a supplied connection string asks for write/create mode. A missing file returns `CUSTOMER.UNAVAILABLE`; a missing customer returns `CUSTOMER.NOT_FOUND`. Invalid data and schema failures return an error, never an empty successful result.
