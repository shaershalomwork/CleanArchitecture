...
`src/Web/Web.http` includes requests with a token placeholder. Default functional tests exercise fake state; the optional database project runs the same HTTP contracts against a disposable SQLite file, with writer/read-only identities and antiforgery tokens. JWT tests verify bearer writes independently of cookies.
